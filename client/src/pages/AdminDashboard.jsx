...
export default function AdminDashboard() {
  ...
  const handleAddProject = async (e) => {
    e.preventDefault();
    if (!projectForm.titleAr || !projectForm.descAr) {
      toast.error('يرجى تعبئة جميع الحقول المطلوبة (العنوان والوصف باللغة العربية كحد أدنى).');
      return;
    }
    ...
    try {
      const payload = {
        ...
        pdfLinks: projectForm.pdfLinks,
        videoLink: projectForm.videoLink
      };

      let addedOrUpdatedProject;
      if (editItem) {
        const { data } = await api.put(`/projects/${editItem._id}`, payload);
        addedOrUpdatedProject = data.data;
        toast.success('تم التعديل بنجاح ✅');
        setProjects(prev => prev.map(p => p._id === addedOrUpdatedProject._id ? addedOrUpdatedProject : p));
      } else {
        const { data } = await api.post('/projects', payload);
        addedOrUpdatedProject = data.data;
        toast.success('تم إضافة المشروع بنجاح.');
        setProjects(prev => [addedOrUpdatedProject, ...prev]);
      }
